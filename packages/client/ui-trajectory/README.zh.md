...
独立 contribution 提供的根 Tool 在完成后仍保留其 Conversation Location。该 Location 解析为 Step 时，根调用及其后代会留在对应 Step 中，不会移动到 Turn 序言。Tool contribution 还可以按调用提供语义显示标签与紧凑行预览；原始调用参数和结果仍可在检查器中查看，而 Tool/Subtool 的种类、颜色、折叠、计时和检查行为保持不变。
